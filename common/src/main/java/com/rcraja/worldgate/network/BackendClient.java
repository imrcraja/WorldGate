...
public final class BackendClient {
    ...
    private static String getPublic(String url){try{HttpRequest r=HttpRequest.newBuilder().uri(URI.create(url)).timeout(Duration.ofSeconds(10)).GET().build();HttpResponse<String> h=CLIENT.send(r,HttpResponse.BodyHandlers.ofString());if(h.statusCode()<200||h.statusCode()>=300)return null;return h.body();}catch(Exception e){return null;}}
