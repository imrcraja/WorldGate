...
public final class BackendClient {
    ...
    public static String eliteProfile(FirebaseSession session, String uid) {
        if (session == null || !session.isReady() || session.idToken() == null) return null;
        String body = "{\"uid\":\"" + escape(uid) + "\"}";
        return request(Constants.BACKEND_BASE_URL + "/v1/elite/profile", session.idToken(), body);
    }

    public static String publicEliteProfile(FirebaseSession session, String uid) {
        if (session == null || !session.isReady() || session.idToken() == null) return null;
        String body = "{\"uids\":[\"" + escape(uid) + "\"]}";
        return request(Constants.BACKEND_BASE_URL + "/v1/elite/public-profiles", session.idToken(), body);
    }

    private static String request(String url, String token, String body) {
        try {
