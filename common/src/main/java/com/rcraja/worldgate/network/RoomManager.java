...
public class RoomManager {
    ...
    public String createRoom(String hostAddress, int hostPort) {
        ...
        String json = "{"
                ...
                + "\"hostPort\":" + hostPort + ","
                + "\"status\":\"waiting\","
                + "\"host\":{\"online\":true,\"lastSeen\":" + System.currentTimeMillis() + "},"
                + "\"player\":{\"online\":false},"
        ...
        if (result == null) {
            WorldGateMod.LOGGER.error("WorldGate room creation failed: {}", roomCode);
            return null;
        }

        WorldGateMod.LOGGER.info("WorldGate room created: {}", roomCode);
        return roomCode;
    }
    ...
    public boolean hostHeartbeat(String roomCode) {
        if (!session.isReady() || roomCode == null) return false;
    ...
    public boolean playerJoin(String roomCode, String ign) {
        if (!session.isReady() || roomCode == null) return false;

        String safeIgn = ign == null ? "Unknown" : ign.replace("\"", "");
        ...
        String result = session.db().patch(
                "/rooms/" + roomCode,
                "{\"status\":\"connected\",\"player\":" + json + "}"
        );
