...
public final class ClaimCenterScreen extends Screen {
    ...
    private void refresh(){status="Syncing...";EliteCoinManager.refresh(()->{for(EliteCoinManager.Mail m:EliteCoinManager.mailbox()){if("UNREAD".equalsIgnoreCase(m.status()))EliteCoinManager.markRead(m.id());}if(minecraft!=null)minecraft.execute(()->status="Synced.");});}
