...
public class WorldGateScreen extends Screen {
    ...
    private void handleRoomChanged(String roomJson) {
        ...
        try {
            ...
            for (String uid : current.keySet()) {
                ...
                boolean wasOnline = knownPlayers.getOrDefault(uid, false);

                if (online && !wasOnline) {
                    String name = playerName(players, uid);
            ...
            for (String uid : knownPlayers.keySet()) {
                ...
                boolean online = current.getOrDefault(uid, false);
    ...
    private String playerName(String uid) {
        ...
        String name =
                playerNameFromRoomJson(roomJson, uid);
    ...
    private void openLink(String url) {
        minecraft.setScreen(
                new ConfirmLinkScreen(
                        confirmed -> minecraft.setScreen(this),
                        url,
                        true
                )
        );
    ...
    private void goBack() {
        ...
        if (room != null && !room.isBlank()) {
            ...
            WorldGateModClient.EXECUTOR.submit(() -> {
                try {
                    WorldGateModClient.ROOM_MANAGER.playerLeave(room);
                } catch (Exception ignored) {
                }

                WorldGateModClient.CURRENT_ROOM_CODE = null;
            });
        }
