...
public final class AccountSecurityScreen extends Screen {
    ...
    @Override protected void init(){
        addRenderableWidget(Button.builder(Component.translatable("worldgate.button.refresh"),b->load())
    ...
    private void load(){
        status=Component.translatable("worldgate.security.loading").getString();
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor g,int mx,int my,float delta){
        ...
        for(int i=0;i<sessions.size();i++){
            JsonObject item=sessions.get(i).getAsJsonObject();
            String name=item.has("deviceName")?item.get("deviceName").getAsString():Component.translatable("worldgate.security.device").getString();
            ...
            String state=active?(current?Component.translatable("worldgate.security.online_this").getString():Component.translatable("worldgate.security.online").getString()):Component.translatable("worldgate.security.logged_in").getString();
            g.text(font,name+" — "+state,width/2-150,y,0xFFFFFFFF);
            if(!id.isBlank())g.text(font,Component.translatable("worldgate.security.session_id",id.substring(0,Math.min(8,id.length()))).getString(),width/2-150,y+14,0xFF8E9AA6);
            y+=28;
