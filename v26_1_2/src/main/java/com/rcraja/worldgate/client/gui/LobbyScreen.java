...
public class LobbyScreen extends Screen {
    ...
    private static class FriendProfile {
        ...
        FriendProfile(String name, String friendCode, boolean online) {
            this.name = name;
    ...
    protected void init() {
        ...
        int chatY = this.height - 42;

        chatBox = new EditBox(
                this.font, 20, chatY, this.width - 110, 20,
        ...
        addRenderableWidget(
                Button.builder(Component.literal("Send"), btn -> sendChat())
                        .bounds(this.width - 85, chatY, 65, 20)
                        .build()
        );

        addRenderableWidget(
                Button.builder(Component.literal("Back"), btn -> goBack())
                        .bounds(centerX - 70, this.height - 20, 140, 20)
                        .build()
        );
        ...
        WorldGateModClient.FRIEND_MANAGER.setFriendListChangedListener(
                json -> {
                    ...
                    if (this.minecraft != null) {
                        this.minecraft.execute(this::refreshFriendProfiles);
                    }
                }
        );
    ...
    private void refreshFriendProfiles() {
        ...
        try {
            ...
            for (String uid : friends.keySet()) {
                WorldGateModClient.EXECUTOR.submit(() -> {
                    String profile =
                            WorldGateModClient.FRIEND_MANAGER.getProfile(uid);

                    String name = "Player";
                    String friendCode = "WG------";
                    ...
                    try {
                        if (profile != null
                            ...
                            if (object.has("displayName")) {
                                name = object.get("displayName").getAsString();
                            }

                            if (object.has("friendCode")) {
                                friendCode = object.get("friendCode").getAsString();
                            }
                    ...
                    final String finalCode = friendCode;
    ...
    private String displayName(String uid) {
        ...
        FriendProfile profile = friendProfiles.get(uid);
        ...
        String roomName = roomPlayerName(uid);
    ...
    private String roomPlayerName(String uid) {
        String json = roomJson;

        if (json == null || json.isBlank()
                || "null".equals(json) || uid == null) {
            return null;
        }
    ...
    private void sendChat() {
        ...
        String text = chatBox.getValue().trim();
    ...
    public void render(
        ...
        graphics.drawCenteredString(
                font, "WorldGate Lobby", centerX, 16, 0xFFFFFF);
        ...
        int leftX = 12;
        int gap = 8;
        int panelWidth = (this.width - 24 - gap * 2) / 3;

        int middleX = leftX + panelWidth + gap;
        int rightX = middleX + panelWidth + gap;

        drawPanel(graphics, leftX, panelTop, panelWidth, bottom - panelTop);
        drawPanel(graphics, middleX, panelTop, panelWidth, bottom - panelTop);
        drawPanel(graphics, rightX, panelTop, panelWidth, bottom - panelTop);

        graphics.drawString(font, "FRIENDS",
                leftX + 10, panelTop + 10, 0x55FFFF);

        graphics.drawString(font, "IN ROOM",
                middleX + 10, panelTop + 10, 0x55FF55);

        graphics.drawString(font, "CHAT",
                rightX + 10, panelTop + 10, 0xFFAA55);

        drawFriends(
                graphics, leftX + 10, panelTop + 28, panelWidth - 20);

        drawRoomPlayers(
                graphics, middleX + 10, panelTop + 28, panelWidth - 20);

        drawChat(
                graphics, rightX + 10, panelTop + 28, panelWidth - 20);

        if (loading) {
            graphics.drawCenteredString(
                    font, "Loading...", centerX, panelTop + 2, 0xAAAAAA);
        }
    ...
    private void drawFriends(
        ...
        if (friendsJson == null
            ...
            graphics.drawString(
                    font, "No friends yet.", x, y, 0x888888);
            return;
        }

        try {
            JsonObject friends =
                    JsonParser.parseString(friendsJson).getAsJsonObject();
            ...
            for (String uid : friends.keySet()) {
                ...
                String code =
                        profile != null ? profile.friendCode : "--------";

                boolean online =
                        profile != null && profile.online;

                int rowY = y + row * 25;

                graphics.drawString(
                        font, name, x, rowY, 0xFFFFFF);

                graphics.drawString(
                        font, code, x, rowY + 10, 0x888888);
            ...
            graphics.drawString(
                    font, "Friends unavailable.", x, y, 0xAAAAAA);
        }
    }
        ...
        if (json == null
            ...
            graphics.drawString(
                    font, "No active room.", x, y, 0x888888);
            return;
        }
        ...
        try {
            ...
            for (String uid : players.keySet()) {
                ...
                if (player.has("ign")) {
                    name = player.get("ign").getAsString();
                }
            ...
            graphics.drawString(
                    font, "Players unavailable.", x, y, 0xAAAAAA);
        }
    }
    ...
    private void drawChat(
        ...
        synchronized (chatMessages) {
            if (chatMessages.isEmpty()) {
                graphics.drawString(
                        font, "No messages yet.", x, y, 0x888888);
                return;
            }

            int row = Math.max(0, chatMessages.size() - 8);
            int drawY = y;

            while (row < chatMessages.size()) {
                String message = chatMessages.get(row);

                graphics.drawString(
                        font, message, x, drawY, 0xFFFFFF);

                drawY += 12;

                if (drawY > this.height - 72) {
                    break;
                }
