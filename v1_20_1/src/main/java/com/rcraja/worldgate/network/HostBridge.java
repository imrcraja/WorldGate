...
public final class HostBridge {
    ...
    public static boolean start(int port) {
        ...
        minecraftPort = port;
        running = true;
        WorldGateMod.LOGGER.info("WorldGate host bridge ready on {}:{}", getAdvertiseAddress(), port);
        return true;
    }
    ...
    public static int getMinecraftPort() { return minecraftPort; }

    public static boolean startRelay(String roomCode) {
        if (!running || minecraftPort <= 0) return false;
        return RelayBridge.startHost(roomCode, minecraftPort);
    }
    ...
    public static String getAdvertiseAddress() {
        try {
            ...
            WorldGateMod.LOGGER.warn("WorldGate: could not detect LAN address", e);
        }
        return "127.0.0.1";
    }
}
